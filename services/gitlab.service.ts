...
export default class GitLab {
    ...
    public getGitLabUserRepos = async (user_id: number) => {
        const { data } = await axios.get(`https://gitlab.com/api/v4/users/${user_id}/projects`, {
            headers: {
                "PRIVATE-TOKEN": `${process.env.GITLAB_PAT}`,
            },
        });
    ...
    public getGitLabUserFollowers = async (user_id: number) => {
        const { data } = await axios.get(`https://gitlab.com/api/v4/users/${user_id}/followers`, {
            headers: {
                "PRIVATE-TOKEN": `${process.env.GITLAB_PAT}`,
            },
        });
    ...
    public getGitLabUserFollowing = async (user_id: number) => {
        const { data } = await axios.get(`https://gitlab.com/api/v4/users/${user_id}/following`, {
            headers: {
                "PRIVATE-TOKEN": `${process.env.GITLAB_PAT}`,
            },
        });
